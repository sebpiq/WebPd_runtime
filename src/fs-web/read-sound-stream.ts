...
import WebPdWorkletNode, { FsRequestReadSoundStream, FsSoundStreamCloseReturn, FsSoundStreamDataReturn, IncomingMessage } from "../WebPdWorkletNode"

const BUFFER_HIGH = 10 * 44100
const BUFFER_LOW = BUFFER_HIGH / 2
const STREAMS: {[operationId: number]: FakeStream} = {}

type ReadSoundStreamMessage = FsRequestReadSoundStream | FsSoundStreamDataReturn | FsSoundStreamCloseReturn
...
const pullBlock = (stream: FakeStream, frameCount: number) => {
    const block = stream.sound.map(
        array => array.slice(stream.readPosition, stream.readPosition + frameCount))
...
const streamLoop = (stream: FakeStream, framesAvailableInEngine: number) => {
    ...
    const framesToSend = BUFFER_HIGH - (framesAvailableInEngine - (secondsToThreshold * sampleRate))
